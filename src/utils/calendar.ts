...
export const generateCalendarMarkings = (
  projects: Project[],
...
export const getProjectColor = (project: Project): string => {
  ...
  switch (project.type) {
    case 'damgeumSoju25':
      return '#20407c'; // 블라블라 색상 기본값
    case 'damgeumSoju30':
      return '#025830'; // 야레야레 색상 기본값
    case 'vodka':
      return '#921e22'; // 계애바 색상 기본값
    default:
      return '#6b7280';
  }
};
...
export const getEventStatusInfo = (
  event: CalendarEvent,
): { status: string; color: string; textColor: string } => {
...
export const calculateProjectStats = (projects: Project[]): ProjectStats => {
  ...
  const recentLogs = projects.reduce((count, project) => {
    return (
      count +
      (project.progressLogs?.filter(
        (log) => log.date && log.date >= oneWeekAgoString && log.date <= today,
      ).length || 0)
    );
  }, 0);
